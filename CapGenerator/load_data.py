...
curr_path = os.getcwd()
print(os.getcwd())
project_dir = os.path.dirname(curr_path) # Image-Captioning
...
def prepare_dataset(data='dev'):
  ...
  if data == 'dev':
    # load dev set (1K)
    filename = project_dir+'/Flickr8k_text/Flickr_8k.devImages.txt'
    ...
    filename = project_dir+'/Flickr8k_text/Flickr_8k.trainImages.txt'
    ...
    # descriptions
    train_descriptions = load_clean_descriptions(project_dir+'/models/descriptions.txt', train)
    test_descriptions = load_clean_descriptions(project_dir+'/models/descriptions.txt', test)
    print('Descriptions: train=%d, test=%d' % (len(train_descriptions), len(test_descriptions)))

    # photo features
    train_features = load_photo_features(project_dir+'/models/features.pkl', train)
    test_features = load_photo_features(project_dir+'/models/features.pkl', test)
    print('Photos: train=%d, test=%d' % (len(train_features), len(test_features)))

  elif data == 'test':
    # load test set
    filename = project_dir+'/Flickr8k_text/Flickr_8k.testImages.txt'
    test = load_set(filename)
